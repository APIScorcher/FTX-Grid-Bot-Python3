...
for i in range(config.NUM_BUY_GRID_LINES):
    ...
    order = exchange.create_limit_buy_order(
        config.SYMBOL, config.POSITION_SIZE, price)
...
while True:
    ...
    for order_id in closed_order_ids:
        buy_orders = [
            buy_order for buy_order in buy_orders if buy_order['id'] != order_id]
        sell_orders = [
            sell_order for sell_order in sell_orders if sell_order['id'] != order_id]
